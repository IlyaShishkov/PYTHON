...
def main():
	...
	while(i < 10): #10 попыток
		...
		if(numUser.isdigit() == True and len(numUser) == 4):
			if (numUser[1:3].find(numUser[0]) != -1 or numUser[0] == numUser[1] or numUser[2:3].find(numUser[1]) != -1 or numUser[0:1].find(numUser[2]) != -1 or numUser[2] == numUser[3] or numUser[0:2].find(numUser[3]) != -1):
				print('Есть повторяющиеся цифры, попытка не засчитана!')
				continue
			for j in range(4):
